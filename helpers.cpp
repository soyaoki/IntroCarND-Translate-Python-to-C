...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	// your code here
  	...
  	vector<float> r1;
  	r1.push_back(corner_prob);r1.push_back(adjacent_prob);r1.push_back(corner_prob);
  	window.push_back(r1);
  	vector<float> r2;
  	r2.push_back(adjacent_prob);r2.push_back(center_prob);r2.push_back(adjacent_prob);
  	window.push_back(r2);
  	window.push_back(r1);
      ...
      for (int j=0;j<w;j++){
        ...
        for (int dx=-1;dx<2;dx++){
          for (int dy=-1;dy<2;dy++){
            ...
            int new_i = ((i+dy) % h + h) % h;
            int new_j = ((j+dx) % w + w) % w;
